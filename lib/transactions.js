...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree){
    ...
    // added forkHash support by Bitcoin Euro
    var forkHash = require(process.cwd() + '/package.json').forkHash;
    var scriptSigPart1;
    if (forkHash) {
      console.log('Added forkHash:', forkHash);
      scriptSigPart1 = Buffer.concat([
        util.reverseBuffer(new Buffer(forkHash + '20', 'hex')),
    ...
    //   util.reverseBuffer(new Buffer('0dc8dbdf87e401ae4ded25bbac54ec3711170efd26f34a9ce9281515246948cd20', 'hex')),
    //   util.serializeNumber(rpcData.height),
    //   new Buffer(rpcData.coinbaseaux.flags, 'hex'),
    //   util.serializeNumber(Date.now() / 1000 | 0),
    //   new Buffer([extraNoncePlaceholder.length]),
